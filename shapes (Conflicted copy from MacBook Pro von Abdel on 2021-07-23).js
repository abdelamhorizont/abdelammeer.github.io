...
function drawLoadedShapes(shape) {
    ...
    loadedShapeColB = map(sin(loadedShapeColCount), 1, -1, loadedCol[2][1], loadedCol[2][0]);
    ...
    for (var i = 0; i < shapeLength; i++) {
        if (shape.layer[0].shapes[0].pos[i] != undefined) {
            ...
            let x = map(noise(shape.layer[0].shapes[0].addPos), 0, 1, -5, 5);
            let y = map(noise(shape.layer[0].shapes[0].addPos), 0, 1, -5, 5);

            // osc = createVector(x, y);
            ...
            if (shape.layer[0].shapes[j].color != undefined) {
                ...
                if (shape.layer[0].shapes[j].color != undefined) {
                    ...
                    if (shapeShapesCol[2][1] != undefined) {
                        shapeShapesColB = map(sin(loadedShapeColCount), 0, 1, shapeShapesCol[2][0], shapeShapesCol[2][1]);
...
function drawAll(shape, layerNum) {

    //softBrushContour(shape);

    //plain shapes in color

    for (var k = 0; k < shape.layer.length; j++) {
        for (var j = 0; j < shape.layer[k].shapes.length; j++) {

            if (shape.layer[k].shapes[j].color != undefined) {
                // shape.layer[layerNum].shapes[j].color[3] = map(sin(frameCount/10), 0, 1, 0.8, 1);
                // fill(shape.layer[layerNum].shapes[j].color);
                ...
                beginShape();
                for (var i = 0; i < shape.layer[k].shapes[j].pos.length; i += 3) {
                    if (shape.layer[k].shapes[j].osc != 0) {
                        shape.layer[k].shapes[j].addPos += shape.layer[k].shapes[j].osc;
                    } else {
                        shape.layer[k].shapes[j].addPos += float(OscSlider.value);
                    }

                    // let x = map(noise(shape.shapes[j].addPos / 10), 0, 1, 0, 5);
                    osc = createVector(sin(shape.layer[layerNum].shapes[j].addPos) * 3, cos(shape.layer[k].shapes[j].addPos) * 3);

                    curveVertex(shape.layer[layerNum].shapes[j].pos[i][0], shape.layer[k].shapes[j].pos[i][1]);

                    shape.layer[k].shapes[j].pos[i][0] += osc.x;
                    shape.layer[k].shapes[j].pos[i][1] += osc.y;
                }
                endShape(CLOSE);
            }
        }
